...
public function __construct($conn="conn"){
     ...
     $this->db     = $GLOBALS['cfg_dbname'];
     $this->conn   = $conn;
     $this->char   = $GLOBALS['cfg_db_language'];
     $this->connect();
    }
...
public function query($sql){
   if($sql== ""){$this->show_error("sql语句错误：","sql查询语句为空");}
    $this->sql=$sql;
    $result=mysql_query($this->sql,$this->conn);
   if(!$result){
    if($this->show_error) $this->show_error("sql语句错误：",$this->sql); // 调试中使用，sql语句出错时会自动打印出来
   }else{
    $this->result=$result;
   }
    return $result;
